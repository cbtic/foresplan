...
function fn_ListarBusqueda() {
	...
	var anio = $('#anio').val();
	var mes = $('#mes').val();
	var rd_tipo = getSelectedRadioValue('rd_tipo');
	$('#id_periodo').val(0);
	
	if(id_subplanilla!=0 && anio!=0 && mes!=0){
		//alert(rd_tipo);
		$.ajax({
			url: '/planilla/obtener_periodo/'+id_ubicacion+'/'+id_planilla+'/'+id_subplanilla+'/'+anio+'/'+mes+'/'+rd_tipo,
			dataType: "json",
			success: function(result){
				//if(result!=null)$('#id_periodo').val(result.id);
				//alert(result);
				$("#tblSolicitud tbody").html("");
				if (result!=null && result.id != "" && result.id_tipo_planilla!=2) {
