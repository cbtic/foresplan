CREATE OR REPLACE FUNCTION public.sp_listar_papeleta_paginado(p_numero_documento character varying, p_persona character varying, p_tipo character varying, p_empresa character varying, p_sede character varying, p_estado character varying, p_pagina character varying, p_limit character varying, p_ref refcursor)
 RETURNS refcursor
 LANGUAGE plpgsql
AS $function$
...
Begin
	...
	If p_sede<>'' Then
	 v_where:=v_where||'And pd.id_sede = '''||p_sede||''' ';
	End If;
