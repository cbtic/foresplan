...
class FieldMaker
{
    ...
    public function make(string $column, array $columnConfig, $object = null)
    {
        ...
        if (! is_null($object)) {
            if (in_array($column, array('id_ubigeo','id_proveedor','id_unidad_medida','id_moneda','id_tipo_producto','id_tipo_documento','id_estado_bien','id_marca','id_um','id_seccion','id_almacen','id_seccion','id_anaquel','id_departamento','id_provincia','id_distrito','id_entrada_productos','id_producto','id_entrada_productos', 'id_estado_productos', 'id_salida_productos'))) {
                $value = $this->getObjectValue($object, $column) ?? $value;
            } elseif (str_contains($column, 'id_')) {
                $value = $this->getObjectValue($object, str_replace("id_", "", $column))->pluck("id")->toArray() ?? $value;
            } else {
                $value = $this->getObjectValue($object, $column) ?? $value;
            }
        }
